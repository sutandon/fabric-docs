...
By default, Power BI uses Microsoft-managed keys to encrypt your data. In Power BI Premium, you can also use your own keys for data at-rest that's imported into a semantic model. This approach is often described as *bring your own key* (BYOK). For more information, see [Data source and storage considerations](#data-source-and-storage-considerations). Power BI BYOK supports double encryption. This includes storage-level encryption using Server-Side Encryption (SSE) and client-side encryption with a Microsoft-managed Data Encryption Key (DEK). This DEK is wrapped using a CMK before data is stored at rest, ensuring layered security and compliance with enterprise encryption standards.
